...
class GreedyAgent(Agent):
    ...
    def action(self) -> int:
        ...
        agent_position = agents_positions[self.agent_id*2:(self.agent_id*2)+2]

        prey_position = self.observation[(self.n_agents*2):]
        
        prey = self.closest_prey(agent_position, prey_position)
        if(prey == None):
            action = np.random.randint(self.n_actions)
        else: 
            action = self.direction_to_go(agent_position, prey)
        return action
